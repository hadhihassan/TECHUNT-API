...
export class ChatController {
    constructor() {
        this._chatUseCase = new ChatUseCase()
    ...
    async sendMessage(req, res) {
        ...
        const result = await this._chatUseCase.createNewMessage(senderId, receiverId, message)
    ...
    async getMessage(req, res) {
        ...
        const senderId = new mongoose.Types.ObjectId(req.clientId);
        const result = await this._chatUseCase.getUserConversation(senderId, userToChatId)
        return res.status(result.status).json(result)
    }
    async getMessagedUser(req, res) {
        const currentUser = req.clientId
        const data = await this._chatUseCase.gteAll(currentUser, req.role)
        return res.status(200).json(data)
    }
    async createConversation(req, res) {
        const { id: receiverId } = req.body
        const senderId = req.clientId;
        const result = await this._chatUseCase.createNewConversation(senderId, receiverId)
